...
# Run
AFL_MAP_SIZE=114985 AFL_NO_AUTODICT=1 \
timeout "${TIMEOUT}" \
  cargo run --release --bin "libaflstar-ftp-${BIN}" -- \
    --in-dir /corpus \
    --out-dir /output_freeopcua \
    --target-port 4840 \
    --loops "${LOOPS}" \
    -t 1000 \
    /freeopcua/build/bin/example_server
...
cp /output_freeopcua/stats.json "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
cp /output_freeopcua/total_stats_info.txt "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
cp -r /output_freeopcua/crashes "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
